...
main(int argc, char **argv)
{
    ...
    while(1) {
        ...
        if(rc > 0) {
            ...
            if(s >= 0 && FD_ISSET(s, &readfds))
                ...
                rc = recvfrom(s6, buf, sizeof(buf) - 1, 0,
                              (struct sockaddr*)&from, &fromlen);
            else if (fifo_fd >= 0 && FD_ISSET(fifo_fd, &readfds)) {
                rc = read(fifo_fd, &req, sizeof(struct server_request));
                flag = 1;
            }
            else
                abort();
        ...
        if (flag == 0) {
            ...
            if (rc != sizeof(struct server_request)) {
                syslog(LOG_NOTICE, "unknown request");
                continue;
            }
            handle_request(req);
        }
